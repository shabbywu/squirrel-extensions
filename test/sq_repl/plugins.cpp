...
std::map<std::string, HMODULE> &get_plugins() { return plugins; }
...
void load_plugins(LauncherContext *ctx) {
  ...
  for (const auto &entry : std::filesystem::directory_iterator(plugin_dir)) {
    ...
    if (!dll) {
      std::cerr << std::format("[!] Loading File<{}> failed!", plugin_name)
                << std::endl;
      continue;
    } else if (!validate_plugin(dll)) {
      std::cerr << std::format("[!] File<{}> is not an validated plugin!",
                               plugin_name)
                << std::endl;

      continue;
    } else {
      std::cerr << std::format("[!] Loaded plugin {}", plugin_name)
                << std::endl;
    }
...
void on_squirrel_vm_init(LauncherContext *ctx) {
  for (const auto [name, entry] : plugins) {
    FARPROC nOffset;
    if (nOffset = GetProcAddress(entry, (LPCSTR) "on_squirrel_vm_init");
        nOffset != NULL) {
      auto call = (void(__cdecl *)(HSQUIRRELVM))nOffset;
      call(ctx->root_vm);
    }

    if (nOffset = GetProcAddress(entry, (LPCSTR) "on_squirrel_vm_init_ext");
        nOffset != NULL) {
