...
static json json_dumps_impl(SQInteger integer) { return integer; }

static json json_dumps_impl(SQFloat floating) { return floating; }

static json json_dumps_impl(bool boolean) { return boolean; }

static json json_dumps_impl(SQChar *str) { return str; }
...
static json json_dumps_impl(detail::Table table) {
  ...
  for (auto [k, v] : table.iterator()) {
    json_object[detail::generic_cast<SQObjectPtr, std::string>(
        *vm, std::forward<SQObjectPtr>(k))] = json_dumps_impl(v);
  }
  return json_object;
}
...
static json json_dumps_impl(SQObjectPtr obj) {
  ...
  switch (obj._type) {
  case OT_ARRAY:
    return json_dumps_impl(detail::generic_cast<SQObjectPtr, detail::Array>(
        *vm, std::forward<SQObjectPtr>(obj)));
  case OT_TABLE: {
    return json_dumps_impl(detail::generic_cast<SQObjectPtr, detail::Table>(
        *vm, std::forward<SQObjectPtr>(obj)));
  }
  case OT_INTEGER: {
...
static std::string json_dumps(SQObjectPtr obj, const int indent = 2,
...
template <typename FromType, typename ToType,
          ...
          typename std::enable_if_t<std::is_same_v<json, std::decay_t<ToType>>>
              * = nullptr>
static ToType generic_cast(detail::VM vm, FromType &&from) {
#ifdef TRACE_OBJECT_CAST
  std::cout << "[TRACING] cast " << typeid(decltype(from)).name() << " to "
