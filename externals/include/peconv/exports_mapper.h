/**
 * @file
 * @brief   A definition of ExportsMapper class. Creates a lookup of all the
 * exported functions from the supplied DLLs. Allows to associate an address
...
struct DllInfo {
  DllInfo() : moduleBase(0), moduelSize(0), is64b(false) {}

  DllInfo(ULONGLONG _moduleBase, size_t _moduelSize, bool _is64b,
...
public:
  /**
  Appends the given DLL to the lookup table of exported functions. Returns the
  number of functions exported from this DLL (not forwarded). \param moduleName
  : name of the DLL \param modulePtr : buffer containing the DLL in a Virtual
  format \param moduleSize : size of the DLL buffer. If moduleSize == 0, the
  ImageSize from the PE headers will be used. \param moduleBase : a base address
  to which the given DLL was relocated
  */
  ...
  number of functions exported from this DLL (not forwarded). \param moduleName
  : name of the DLL \param modulePtr : buffer containing the DLL in a Virtual
  format \param moduleBase : a base address to which the given DLL was relocated
  */
  size_t add_to_lookup(std::string moduleName, HMODULE modulePtr,
                       ULONGLONG moduleBase) {
  ...
  /**
  Appends the given DLL to the lookup table of exported functions. Returns the
  number of functions exported from this DLL (not forwarded). Assumes that the
  module was relocated to the same address as is the address of the given buffer
  (modulePtr). (A wrapper for the case if we are adding a DLL that was loaded
  within the current process.) \param moduleName : name of the DLL \param
  modulePtr : buffer containing the DLL in a Virtual format.
  */
  size_t add_to_lookup(std::string moduleName, HMODULE modulePtr) {
    return add_to_lookup(moduleName, modulePtr,
                         reinterpret_cast<ULONGLONG>(modulePtr));
  }

  /**
  Find the set of Exported Functions that can be mapped to the given VA.
  Includes forwarders, and function aliases.
  */
  ...
  /**
  Retrieve the base of the DLL containing the given function. If not found,
  returns 0.
  */
  ULONGLONG find_dll_base_by_func_va(ULONGLONG func_rva) const {
    // the first element that is greater than the start address
    std::map<ULONGLONG, DllInfo>::const_iterator firstGreater =
    ...
    for (itr = dll_base_to_info.begin(); itr != firstGreater; ++itr) {
      ...
      if (func_rva >= module.moduleBase &&
          func_rva <= (module.moduleBase + module.moduelSize)) {
  ...
  */
  std::string get_dll_path(ULONGLONG base) const {
    std::map<ULONGLONG, DllInfo>::const_iterator found =
        this->dll_base_to_info.find(base);
  ...
  /**
  Retrieve the path of the DLL with the given short name. If multiple paths are
  mapped to the same short name, it retrieves the first one.
  */
  ...
  */
  size_t get_dll_paths(IN std::string short_name,
                       OUT std::set<std::string> &paths) const;

  /**
  Retrieve the full name of the DLL (including the extension) using its short
...
private:
  ...
  ADD_FUNC_RES add_function_to_lookup(HMODULE modulePtr, ULONGLONG moduleBase,
                                      size_t moduleSize, ExportedFunc &currFunc,
                                      DWORD callRVA);

  bool add_forwarded(ExportedFunc &currFunc, DWORD callRVA, PBYTE modulePtr,
                     size_t moduleSize);
  bool add_to_maps(ULONGLONG va, ExportedFunc &currFunc);

  size_t resolve_forwarders(const ULONGLONG va, ExportedFunc &currFunc);
  size_t make_ord_lookup_tables(PVOID modulePtr, size_t moduleSize,
