/**
 * @file
 * @brief   Functions related to hooking the loaded PE. Reditecting/replacing a
...
/**
A buffer storing a binary patch, that can be applied on a module. Used as a
restorable backup in case of function patching.
*/
class PatchBackup {
public:
  /**
  ...
  */
  PatchBackup() : buffer(nullptr), bufferSize(0), sourcePtr(nullptr) {}

  ~PatchBackup() { deleteBackup(); }
  ...
  */
  ...
  /**
  Reads bytes from the binary to the backup. The source buffer must be within
  the current process.
  */
  ...
  */
  bool isBackup() { return buffer != nullptr; }
...
/**
A functions resolver that can be used for hooking IAT. Allows for defining
functions that are supposed to be replaced.
*/
...
public:
  ...
  */
  ...
  If the function was hooked, it retrieves the address of the replacement
  function instead. \param func_name : the name of the function \param lib_name
  : the name of the DLL \return Virtual Address of the exported function, or the
...
\return size of the applied patch
*/
size_t redirect_to_local(void *ptr, void *new_function_ptr,
                         PatchBackup *backup = nullptr);
