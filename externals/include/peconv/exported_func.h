/**
 * @file
 * @brief   A definition of ExportedFunc  class - used for storing the details
 * of the exported function. Helper functions related to the export parsing.
 */
...
/**
Check if the given string is in a format typical for storing ordinals
(#[ordinal])
*/
...
/**
Convert the function in a format: DLL_name.function_name into a normalized form
(DLL name in lowercase).
*/
...
public:
  ...
  */
  ...
  //! Compares functions' names. If function is defined by an ordinal, compares
  //! ordinals. Does not include the DLL name in the comparison.
  static bool isTheSameFuncName(const peconv::ExportedFunc &func1,
                                const peconv::ExportedFunc &func2);

  //! Compares functions' DLL names.
  static bool isTheSameDllName(const peconv::ExportedFunc &func1,
                               const peconv::ExportedFunc &func2);

  //! Compares functions' names. If function is defined by an ordinal, compares
  //! ordinals. Includes the DLL name in the comparison.
  static bool isTheSameFunc(const peconv::ExportedFunc &func1,
                            const peconv::ExportedFunc &func2);
  ...
  Gives the priority to the named functions: if one of the compared functions is
  unnamed, the named one is treated as smaller. If both functions are unnamed,
  the function with the smaller ordinal is treated as smaller. Otherwise, the
  function with the shorter name is treated as smaller.
  */
  ...
  /**
  Gets a string representation of the variable. Full info about the function:
  library, name, ordinal.
  */
  std::string toString() const;

  /**
  Gets a string representation of the variable. Short info about the function:
  only function name or ordinal (if the name is missing).
  */
  std::string nameToString() const;

  bool isValid() const { return (funcName != "" || funcOrdinal != -1); }
};
